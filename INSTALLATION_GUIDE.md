...
# Option B — Run docker build directly
...
```bash
# Navigate to the workshop root (one level above docker/)
cd ..
...
docker run --rm -it -v $(pwd):/workspace opensta-workshop
```
...
### 4.3 Using Docker Compose (Alternative)

If you prefer Docker Compose, a `docker-compose.yml` is provided:
...
docker build --no-cache -f Dockerfile.linux -t opensta-workshop .
```
...
| Command | Description |
|---|---|
| `docker build -f Dockerfile.linux -t opensta-workshop .` | Build the image |
| `docker run --rm -it -v $(pwd):/workspace opensta-workshop` | Run an interactive container |
| `docker run --rm -v $(pwd):/workspace opensta-workshop sta script.tcl` | Run a script and exit |
